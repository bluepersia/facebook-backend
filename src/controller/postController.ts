...
{
    const friends = await factory.getAllFriends ((req as IRequest).user._id, true);
...
export const processImages = handle (async (req:Request, res:Response, next:() => void): Promise<void> =>
{
    if (req.files)
    {
        ...
        const images = files.map ((img, i) => `img-<SIZE>-${Date.now()}-${i}.jpeg`);
        ...
        await Promise.all (files.map (async (img, i) => {
